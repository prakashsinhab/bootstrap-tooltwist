...
public class NavBarWidget extends ContainerWidget {
	...
	public void getCodeInserters(WbdGenerator generator, WbdWidget instance, UimData ud, CodeInserterList codeInserterList) throws WbdException {
		...
		if (mode == GenerationMode.DESIGN) {
			...
			CodeInserter[] arr = {
					...
					new StylesheetCodeInserter(generator, instance, "navBar_cssHeader.css"), new StylesheetLinkInserter(ToolTwist.getWebapp() + "/bootstrap/css/bootstrap.min.css?v=3.0.0"),
			// new
			// StylesheetLinkInserter("/ttsvr/bootstrap/css/bootstrap.min.css"),
			// new JavascriptCodeInserter(generator, instance,
			// "navBar_jsHeader.js"),
			// new
			// JavascriptLinkInserter("/ttsvr/bootstrap/js/bootstrap.min.js")
			};
			...
			if (USE_PRODUCTION_HELPER) {
				...
				// NavBarProductionHelper.class.getName(),
				// productionHelperParams));
				// codeInserterList.add(new
				// PageImportCodeInserter(NavBarProductionHelper.class.getName()));
			}
		}
			...
			for (int row = 0; row < Integer.valueOf(rows); row++) {
				...
				if (type.equals("Link")) {
					...
					if (isDisplay) {
						...
						// linkNavpoint = RoutingUIM.navpointUrl(ud,
						// instance.getProperty("linkNavpoint", wbdChildIndex),
						// null);
					...
					if (isDisplay) {
						// rh.append("        <a href=" + linkNavpoint +
						// parameters + "><form class=\"navbar-form " +
						// horizontalPositionClass +
						// "\"><button type=\"button\" class=\"btn "
						// + buttonTypeClass + " " + buttonSizeClass +
						// "\"><span class=\"glyphicon " + buttonGlyphiconClass
						// + "\"></span>&nbsp;" + title +
						// "</button></form></a>\n");
						rh.append("        <a href=" + linkNavpoint + parameters + "><form class=\"navbar-form " + horizontalPositionClass + "\">" + "<span class=\"btn " + buttonTypeClass + " "
								+ buttonSizeClass + "\">" + "<span class=\"glyphicon " + buttonGlyphiconClass + "\"></span>&nbsp;" + title + "</span></form></a>\n");
					}
				}
			}
	...
	private void render(WbdGenerator generator, WbdWidget instance, UimData ud, WbdRenderHelper rh) {
		...
		try {
			...
			for (int row = 0; row < Integer.valueOf(rows); row++) {
				...
				rh.append("          <li class=\"" + clazz + " designer-properties\"  id=\"" + navBarId + "[" + NAVBAR_INDEX_PREFIX + row + "]" + "\" onclick=\"NavBar.selectNavBar('"
						+ navBarId.fullPath() + "','" + row + "')\"><a href=\"javascript:void(0);\">" + title + "</a></li>\n");
	...
	public void renderProperties(WbdGenerator generator, UimData ud, WbdRenderHelper rh, WbdWidget instance, WidgetId id, boolean displayOnly) throws WbdException {
		...
		rh.append("<tr>");
		rh.append("<td id=\"id-designer-properties-navBar3\">");// [elementId,
																// rows,
																// selectedRow,
																// position,
																// inverted,
		...
		rh.append("<td id=\"id-designer-properties-navBar3\">");// [elementId,
																...
																// _controller,
																// _linkedWidget]
		String[] _linkedWidgetOnly = { "elementId", "rows", "selectedRow", "brandType", "titleImagePath", "brandNavpoint", "verticalPosition", "inverted", "_widgetId", "_controller", "_controller",
				"_widgetId", "cellDivs", "rows", "selectedRow", "title" };
		...
		if (instance.mayEdit(ud) && canChangeGrid) {
			...
			rh.append("  <span class=\"button-navBar\" style=\"float: right;cursor: pointer;\" onclick=\"NavBar.removeNavBar('" + id.fullPath()
					+ "');\" title=\"Decrease row count.\">&nbsp;-&nbsp;</span>");
			rh.append("  <span class=\"button-navBar\" style=\"float: right;cursor: pointer;\" onclick=\"NavBar.insertNavBar('" + id.fullPath()
					+ "');\" title=\"Increase row count.\">&nbsp;+&nbsp;</span>");
		}
